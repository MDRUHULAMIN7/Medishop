...
export function useProductFilters(categorySlug?: string) {
  ...
  const queryResult = useQuery({
    queryKey: ['products-listing', categorySlug, searchParams.toString()],
    queryFn: () => ProductService.getProducts(filterState, sort, searchQuery),
    staleTime: 60 * 1000,
  });
  ...
  const setCategories = (cats: string[]) =>
    updateURL({ categories: cats.length > 0 ? cats.join(',') : null });

  const setBrands = (b: string[]) =>
    updateURL({ brands: b.length > 0 ? b.join(',') : null });
  ...
  const setDiscounts = (disc: number[]) =>
    updateURL({ discounts: disc.length > 0 ? disc.join(',') : null });

  const setInStockOnly = (val: boolean) =>
    updateURL({ inStock: val ? 'true' : null });

  const setPrescriptionReq = (val: 'all' | 'required' | 'otc') =>
    updateURL({ rx: val === 'all' ? null : val });

  const setSort = (s: SortOption) => updateURL({ sort: s });
